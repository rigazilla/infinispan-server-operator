...
func (c *configResource) Process() (reconcile.Result, error) {
	...
	if c.infinispan.HasSites() {
		...
		if result, err := kube.LookupResource(c.infinispan.GetSiteServiceName(), c.infinispan.Namespace, siteService, c.infinispan, c.client, c.log, c.eventRec); result != nil {
			return *result, err
		}
